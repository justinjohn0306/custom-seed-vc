...
"""Gradio application interface for OpenVoice."""
...
def predict(
    ...
    """Process voice cloning request with the given text prompt and reference audio.

    Args:
        prompt: Text to be synthesized (up to 200 characters).
        style: Voice style for synthesis. Options include 'default', 'whispering',
            'shouting', 'excited', 'cheerful', 'terrified', 'angry', 'sad', 'friendly'.
            Chinese only supports 'default'.
        audio_file_pth: Path to the reference audio file for voice cloning.
        agree: Whether user agrees to the license terms.

    Returns:
        A tuple containing:
            - text_hint: Status message or error information.
            - save_path: Path to the generated audio file (None if failed).
            - speaker_wav: Path to the reference audio used (None if failed).
    """
