...
"""Autoregressive model implementation for Seed-VC v2."""
...
def find_multiple(n: int, k: int) -> int:
    ...
    Args:
        n: The minimum value.
        k: The value to find multiples of.

    Returns:
        The smallest multiple of k that is >= n.
    """
    if n % k == 0:
        return n
    return n + k - (n % k)
...
def l2norm(t: torch.Tensor, groups: int = 1) -> torch.Tensor:
    """Apply L2 normalization to tensor with optional grouping.
...
@dataclass
class BaseModelArgs:
    """Base configuration for autoregressive transformer models.

    Contains all hyperparameters needed to initialize transformer models
    including architecture details, optimization settings, and features.
    """
...
class NaiveModelArgs(BaseModelArgs):
    """Configuration for naive autoregressive transformer models.

    Extends BaseModelArgs with specific settings for naive transformer
    implementation used in voice conversion tasks.
    """
...
class KVCache(nn.Module):
    ...
    Attributes:
        k_cache: Buffer for key cache.
        v_cache: Buffer for value cache.
    """
    ...
    def update(
        ...
        Args:
            input_pos: Positions to update [S].
            k_val: Key values [B, H, S, D].
            v_val: Value values [B, H, S, D].

        Returns:
            Updated key and value caches.
        """
...
class TransformerForwardResult:
    """Result container for transformer forward pass.

    Holds the model outputs including logits and targets for
    computing losses during training.
    """
...
class BaseTransformerForwardResult:
    """Result container for base transformer forward pass.

    Contains the raw logits and hidden states from the transformer
    before task-specific processing.
    """
...
class BaseTransformer(nn.Module):
    """Base transformer model for autoregressive generation.
    ...
    def setup_caches(
        ...
        """Setup key-value caches for efficient inference.

        Args:
            max_batch_size: Maximum batch size.
            max_seq_len: Maximum sequence length.
            dtype: Data type for caches.
            device: Device to store caches.
        """
    ...
    def embed_base(self, x: Tensor, x_lens: Tensor) -> Tuple[Tensor, Tensor]:
        """Embed input tokens and pad with special tokens.

        Args:
            x: Input token ids.
            x_lens: Lengths of each sequence in batch.

        Returns:
            Tuple of (padded token ids, embeddings).
        """
    ...
    def forward(
        ...
        Returns:
            Model output with logits and hidden states.
        """
        seq_len = inp.size(1)
    ...
    def forward_generate(
        ...
        Args:
            inp: Input tensor.
            input_pos: Input position indices.
            kv_pos: Key-value position indices.
            return_all: Whether to return all hidden states.
...
class NaiveTransformer(BaseTransformer):
    """Naive transformer implementation for voice conversion."""

    def __init__(self, config: NaiveModelArgs) -> None:
        """Initialize naive transformer.
    ...
    def forward(
        ...
        """Forward pass through naive transformer.

        Args:
            inp: Input tensor.
            cond_lens: Conditioning lengths.
        ...
        Returns:
            Model output with logits, loss, and accuracy.
        """
    ...
    def infer_slow(self, inp: Tensor, input_pos: Optional[Tensor] = None) -> Tensor:
        """Run slow inference without KV cache.
...
class NaiveWrapper(nn.Module):
    """Wrapper for naive transformer with conditioning support.

    Attributes:
        model: The underlying transformer model.
        sep_token_emb: Separator token embedding.
    """
    ...
    def __init__(self, model: NaiveTransformer) -> None:
        ...
        Args:
            model: Naive transformer model to wrap.
        """
        super().__init__()
        self.model = model
    ...
    def setup_caches(
        ...
        """Setup key-value caches for the model.
    ...
    def forward(self, cond: Tensor, cond_lens: Tensor, x: Tensor, x_lens: Tensor) -> torch.Tensor:
        ...
        Args:
            ...
            cond_lens: Lengths of conditioning sequences.
            x: Input token ids.
    ...
    def generate(
        self,
        prompt_text: Tensor,
        prompt_target: Tensor,
        compiled_decode_fn: Optional[callable] = None,
        **sampling_kwargs,
    ) -> Tensor:
        """Generate tokens with prompting.

        Args:
            prompt_text: Text prompt embeddings.
            prompt_target: Target prompt tokens.
            compiled_decode_fn: Optional compiled decode function.
        ...
        input_pos = torch.cat(
            [input_pos, torch.arange(prompt_target_emb.size(1)).to(input_pos.device) + 1],
        )
        ...
        new_emb = self.model.embed_base(
            pred_base.unsqueeze(0),
            torch.LongTensor([1]).to(pred_base.device),
        )[1]
        emb_seq = torch.cat([emb_seq, new_emb], dim=1)
    ...
    def decode_one_token_ar(
        ...
        **sampling_kwargs,
        ...
        """Decode one token using autoregressive generation.

        Args:
            x: Input tensor.
            input_pos: Input position.
            kv_pos: Key-value position.
            previous_tokens: Previous generated tokens.
            compiled_decode_fn: Compiled decode function.
...
class TransformerBlock(nn.Module):
    ...
    Attributes:
        ...
        ffn_norm: Normalization for FFN.
        attention_norm: Normalization for attention.
    """
    ...
    def forward(
        ...
        Args:
            ...
            mask: Attention mask.
            input_pos: Input positions for caching.
...
class Attention(nn.Module):
    ...
    def load_hook(self, state_dict: dict, prefix: str, *args) -> None:
        """Hook for loading separate q,k,v weights.
    ...
    def eq_scaled_dot_product_attention(
        ...
        """Equivalent scaled dot product attention implementation.

        This is a standard scaled dot product attention.
        It's less efficient, but it doesn't raise cuda errors.
...
def precompute_freqs_cis(seq_len: int, n_elem: int, base: int = 10000) -> Tensor:
    ...
    Args:
        ...
        base: Base for exponential scaling.
...
def top_k_top_p_filtering(
    ...
    Args:
        ...
        top_k: If > 0, keep only top k tokens with highest probability (top-k filt.).
        top_p: If < 1.0, keep the top tokens with cumulative probability >= top_p (nucleus filt.).
            Nucleus filtering is described in Holtzman et al. (http://arxiv.org/abs/1904.09751)
        filter_value: Value to assign to filtered out tokens.
        min_tokens_to_keep: Minimum tokens to keep per batch example in the output.
...
def topk_sampling(
    ...
    """Sample from logits using top-k and top-p filtering.

    Args:
        logits: Logits to sample from.
        top_k: Number of highest probability tokens to keep for top-k-filtering.
            Between 1 and infinity. Default to 10.
        top_p: Cumulative probability of highest probability tokens to keep for nucleus sampling.
            Must be between 0 and 1. Default to 1.
        temperature: Value used to modulate the next token probabilities.
            Must be strictly positive. Default to 1.0.

    Returns:
        Tuple of (sampled token, log probability).
    """
...
def sample(
    ...
    **sampling_kwargs,
    ...
    Returns:
        Tuple of (sampled token index, probabilities).
    """
...
def logits_to_probs(
    ...
    temperature: Union[float, Tensor] = 0.7,
    top_p: Union[float, Tensor] = 0.7,
    repetition_penalty: Union[float, Tensor] = 1.5,
) -> torch.Tensor:
    """Convert logits to probabilities with various sampling strategies.
    ...
    Args:
        ...
        repetition_penalty: Penalty for repeating previous tokens.
