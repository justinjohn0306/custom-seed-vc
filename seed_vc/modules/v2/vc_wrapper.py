...
"""Voice conversion wrapper for Seed-VC v2."""
...
class VoiceConversionWrapper(torch.nn.Module):
    """Wrapper for voice conversion models.

    Integrates multiple components for voice conversion including content extraction,
    style encoding, AR generation, and vocoder synthesis.
    ...
    Attributes:
        ...
        cfm_length_regulator: Length regulator for CFM.
        content_extractor_narrow: Narrow-band content extractor.
        content_extractor_wide: Wide-band content extractor.
        ar_length_regulator: Length regulator for AR model.
        ar: Autoregressive model.
        style_encoder: Style encoder model.
        vocoder: Vocoder for waveform synthesis.
        overlap_frame_len: Overlap length in frames for streaming.
        bitrate: Bitrate for MP3 encoding.
        compiled_decode_fn: Compiled decode function for AR model.
        dit_compiled: Whether DiT is compiled.
        dit_max_context_len: Maximum context length for DiT in seconds.
        ar_max_content_len: Maximum content length for AR model.
        compile_len: Compilation length.
    """
    ...
    def __init__(
        ...
        """Initialize voice conversion wrapper.
    ...
    def forward_cfm(
        self,
        content_indices_wide: torch.Tensor,
        content_lens: torch.Tensor,
        mels: torch.Tensor,
        mel_lens: torch.Tensor,
        style_vectors: torch.Tensor,
    ) -> torch.Tensor:
        """Forward pass for CFM model.

        Args:
            content_indices_wide: Wide-band content indices.
            content_lens: Content lengths.
            mels: Mel-spectrograms.
            mel_lens: Mel-spectrogram lengths.
            style_vectors: Style embeddings.
    ...
    def forward_ar(
        self,
        content_indices_narrow: torch.Tensor,
        content_indices_wide: torch.Tensor,
        content_lens: torch.Tensor,
    ) -> torch.Tensor:
        """Forward pass for AR model.

        Args:
            content_indices_narrow: Narrow-band content indices.
            content_indices_wide: Wide-band content indices.
            content_lens: Content lengths.
    ...
    def forward(
        ...
        mel_lens: torch.Tensor,
        forward_ar: bool = False,
        forward_cfm: bool = True,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Forward pass for the model.

        Args:
            waves_16k: Input waveforms at 16kHz.
            mels: Mel-spectrograms.
    ...
    def compile_cfm(self) -> None:
        """Compile the CFM model for inference."""
    ...
    @staticmethod
    def duration_reduction_func(
        token_seq: torch.Tensor,
        n_gram: int = 1,
    ) -> Tuple[torch.Tensor, int]:
        """Reduce token sequence by removing consecutive duplicates.

        Args:
            token_seq: Input token sequence [T].
            n_gram: N-gram size for reduction.

        Returns:
            Tuple of:
                - reduced_token_seq: Reduced token sequence [T'].
                - reduced_token_seq_len: Length of reduced sequence.
        """
        n_gram_seq = token_seq.unfold(0, n_gram, 1)
        mask = torch.all(n_gram_seq[1:] != n_gram_seq[:-1], dim=1)
        reduced_token_seq = torch.cat((n_gram_seq[0, :n_gram], n_gram_seq[1:, -1][mask]))
    ...
    def _stream_wave_chunks(
        ...
        Returns:
            Tuple of:
                - processed_frames: Updated processed frames count.
                - previous_chunk: Updated previous chunk.
                - should_break: Whether processing should stop.
                - mp3_bytes: MP3 bytes if streaming, None otherwise.
                - full_audio: Full audio tuple if last chunk, None otherwise.
        """
        mp3_bytes = None
        full_audio = None
    ...
    def load_checkpoints(
        ...
        Args:
            cfm_checkpoint_path: Path to CFM checkpoint.
            ar_checkpoint_path: Path to AR checkpoint.
        """
        ...
        cfm_length_regulator_state_dict = self.strip_prefix(
            cfm_checkpoint["net"]["length_regulator"],
            "module.",
        )
        ...
        missing_keys, unexpected_keys = self.cfm_length_regulator.load_state_dict(
            cfm_length_regulator_state_dict,
            strict=False,
        )
        ...
        ar_length_regulator_state_dict = self.strip_prefix(
            ar_checkpoint["net"]["length_regulator"],
            "module.",
        )
        ...
        missing_keys, unexpected_keys = self.ar_length_regulator.load_state_dict(
            ar_length_regulator_state_dict,
            strict=False,
        )
        ...
        self.content_extractor_narrow.load_state_dict(
            content_extractor_narrow_checkpoint,
            strict=False,
        )
        ...
        self.content_extractor_wide.load_state_dict(content_extractor_wide_checkpoint, strict=False)

        # style encoder
        style_encoder_checkpoint_path = load_custom_model_from_hf(
            DEFAULT_SE_REPO_ID,
            DEFAULT_SE_CHECKPOINT,
    ...
    def setup_ar_caches(
        ...
        """Set up AR model caches for efficient inference.

        Args:
            max_batch_size: Maximum batch size.
            max_seq_len: Maximum sequence length.
            dtype: Data type for caches.
            device: Device for caches.
        """
    ...
    def compute_style(
        self,
        waves_16k: torch.Tensor,
        wave_lens_16k: Optional[torch.Tensor] = None,
    ) -> torch.Tensor:
        """Compute style embeddings from waveforms.

        Args:
            waves_16k: Input waveforms at 16kHz.
            wave_lens_16k: Optional waveform lengths.
    ...
    def convert_timbre(
        ...
        """Convert timbre from source to target audio.
        ...
        Args:
            ...
            use_sway_sampling: Whether to use sway sampling.
            use_amo_sampling: Whether to use AMO sampling.
    ...
    def convert_voice(
        ...
        with torch.autocast(device_type=device.type, dtype=dtype):
            ...
            cond, _ = self.cfm_length_regulator(
                ar_out,
                ylens=torch.LongTensor([ar_out_mel_len]).to(device),
            )
            (
                prompt_condition,
                _,
    ...
    def _process_content_features(
        ...
        Args:
            audio_16k_tensor: Audio tensor at 16kHz.
            is_narrow: Whether to use narrow-band extractor.

        Returns:
            Content indices tensor.
        """
    ...
    def convert_voice_with_streaming(
    ...
    ) -> Union[Generator[Tuple[bytes, Optional[Tuple[int, np.ndarray]]], None, None], np.ndarray]:
        ...
        Args:
            ...
            diffusion_steps: Number of diffusion steps.
            length_adjust: Length adjustment factor.
            intelligebility_cfg_rate: CFG rate for intelligibility.
            similarity_cfg_rate: CFG rate for similarity.
            top_p: Top-p sampling parameter.
            temperature: Temperature for sampling.
            repetition_penalty: Repetition penalty.
            convert_style: Whether to convert style.
            anonymization_only: Whether to only anonymize voice.
            device: Device to use.
            dtype: Data type to use.
            stream_output: Whether to stream the output.
